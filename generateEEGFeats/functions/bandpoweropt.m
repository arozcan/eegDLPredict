function pwr = bandpoweropt(varargin)
...
function pwr = psdbandpower(Pxx, W, freqrange, freqrangefilter)
...
idx1=zeros(size(freqrange,1),1);
idx2=zeros(size(freqrange,1),1);
for i=1:size(freqrange,1)
    idx1(i) = find(W<=freqrange(i,1), 1, 'last' );
    idx2(i) = find(W>=freqrange(i,2), 1, 'first');
end
...
if nargin > 3
    % Find indices of filtered freq range requested.
    for i=1:size(freqrangefilter,1)
        idx1_filt = find(W<=freqrangefilter(i,1), 1, 'last' );
        idx2_filt = find(W>=freqrangefilter(i,2), 1, 'first');
        width(idx1_filt:idx2_filt) = 0;
    end
end

% Sum the average power over the range of interest.
pwr=zeros(size(freqrange,1),1);
for i=1:size(freqrange,1)
    pwr(i) = width(idx1(i):idx2(i))'*Pxx(idx1(i):idx2(i),:);
end
